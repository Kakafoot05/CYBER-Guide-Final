...
const getProjectBriefing = (project: Project, locale: 'fr' | 'en'): ProjectBriefing => {
  ...
  return {
    ...
    operationalValue:
      project.takeaways?.[0] ??
      (locale === 'en'
        ? 'Strengthens operational cybersecurity capabilities.'
        : 'Renforce les capacités de cybersécurité opérationnelle.'),
    workflow,
  };
};
...
const Projects: React.FC = () => {
  ...
  const { projects } = getLocalizedContent(locale);
  ...
  const copy = isEnglish
    ? {
        ...
        communityTitle: 'Community call: propose a project case study',
        communityBody:
          'You can submit a real operational project to be documented in Cyber Guide and shared with the community.',
        communityCriteriaTitle: 'To be selected, your project should include:',
        communityCriteria: [
          'A clear operational context (incident, hardening, cloud, governance, etc.)',
          'At least 2 official or vendor sources',
          'A measurable before/after result (time, coverage, risk reduction)',
          'Reusable lessons for other French organizations',
        ],
        communityHint:
          'Priority is given to practical projects with measurable outcomes and reliable references.',
        communityButton: 'Propose a project',
        completed: 'COMPLETED',
        pipelineSteps: [
    ...
    : {
        ...
        communityTitle: 'Appel communautaire : proposer un dossier projet',
        communityBody:
          'Vous pouvez soumettre un projet opérationnel réel pour qu’il soit documenté dans Cyber Guide et partagé à la communauté.',
        communityCriteriaTitle: 'Pour être retenu, votre projet doit inclure :',
        communityCriteria: [
          'Un contexte opérationnel clair (incident, durcissement, cloud, gouvernance, etc.)',
          'Au moins 2 sources officielles ou éditeur',
          'Un résultat mesurable avant/après (temps, couverture, réduction de risque)',
          "Des enseignements réutilisables pour d'autres organisations françaises",
        ],
        communityHint:
          'Priorité aux projets pratiques avec résultats mesurables et références fiables.',
        communityButton: 'Proposer un projet',
        completed: 'TERMINÉ',
        pipelineSteps: [
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-32">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <section className="mb-10 rounded-sm border border-brand-navy/15 bg-white p-6 shadow-panel md:p-7">
          <div className="grid gap-6 md:grid-cols-[1.2fr_1fr] md:items-start">
            <div>
              <div className="mb-3 inline-flex items-center gap-2 rounded-sm border border-brand-steel/30 bg-brand-pale/40 px-2.5 py-1 text-[10px] font-mono uppercase tracking-widest text-brand-navy">
                <ShieldCheck size={12} />
                Community
              </div>
              <h2 className="text-2xl font-display font-bold text-brand-navy">{copy.communityTitle}</h2>
              <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-600">{copy.communityBody}</p>
              <p className="mt-3 text-xs text-slate-500">{copy.communityHint}</p>
            </div>

            <div className="rounded-sm border border-slate-200 bg-slate-50 p-4">
              <p className="text-[11px] font-bold uppercase tracking-wider text-brand-navy">
                {copy.communityCriteriaTitle}
              </p>
              <ul className="mt-3 space-y-2 text-sm text-slate-700">
                {copy.communityCriteria.map((criterion) => (
                  <li key={criterion} className="flex items-start gap-2">
                    <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-brand-steel" />
                    <span>{criterion}</span>
                  </li>
                ))}
              </ul>
